-- ============================================================================
-- Migration: Limit confirm_rental_start to generate 1 future pending payment
-- Date: 2026-04-05
--
-- Problem: confirm_rental_start was generating ~6 weeks of pending payments
--          upfront (CURRENT_DATE + 42 days lookahead). This is the cron's job.
--
-- Fix: Reduce lookahead to CURRENT_DATE + 7 days so only 1 pending payment
--      is created. The generate_weekly_payments cron handles all future weeks.
-- ============================================================================
...
BEGIN
  ...
  --    • Lookahead: CURRENT_DATE + 7 days (1 pending payment max)
  --    • Cron (generate_weekly_payments) handles all future weeks
  -- ------------------------------------------------------------------
